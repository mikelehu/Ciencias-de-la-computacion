...
# ╔═╡ 10057a12-8f7c-4b49-afde-843cb924735a
using Plots,PlutoUI

# ╔═╡ b40d69fa-3cef-11ed-0a63-d10cb709228c
md"""
# Velocidad de caida libre de una pelota
"""

# ╔═╡ b212196e-995d-4127-93bc-d5af1602aa2f
md"""
...
# ╔═╡ 353a4ce0-a121-4f4a-99a3-bea63310f64f
PlutoUI.TableOfContents()

# ╔═╡ 70fe9962-2bcf-4974-a47a-83241b03f595
md"""
## 1. Modelo simplificado
"""

# ╔═╡ 83a20079-009b-48d9-b955-f3704ec4dd0f
md"""
...
# ╔═╡ 76a5e826-4caa-4e50-a6d2-7ff48938189b
md"""
...
# ╔═╡ f6f6c5b2-80a7-459c-8ffb-8f041c3f2237
md"""
>- **¿Cual es la variable de estado?**   v
>- **¿Cual es la variable independiente?**  t
>- **¿Cuales son los parámetros constantes del problema?**   g,M,c
"""

# ╔═╡ 90a5db6b-d2ed-481e-a767-5516185a42ba
md"""
...
# ╔═╡ f8077c0d-fe29-4c56-94d4-0bf8530e3a7b
md"""
...
# ╔═╡ e6805196-396b-46c0-b98d-92592009b9a7
md"""
...
# ╔═╡ 2b2cf3c9-e320-4660-92ef-0fb8fe0d7d1d
begin
g = 9.8
M = 0.058
c = 29/90000
...
# ╔═╡ 863d3592-3e2a-4a2b-8cf6-4bdfef7c99fc
md"""
>**Respuesta**
>
>La velocidad terminal de la pelota de tenis es de 42.0 m/s.

"""

# ╔═╡ aa947f9e-fa36-4718-a95b-c4c6917c95c7
md"""
### 1.3- Ejercicio
...
# ╔═╡ 0b2f9843-d9f0-4085-b428-bb35677ebc00
function vsol(t,vT)
    aux = exp(-2g*t/vT)   #una variable auxiliar para evitar repetir cálculos 
    v = -vT*(1-aux)/(1+aux)
    return v
end

# ╔═╡ d6335a00-99df-41c3-add0-c05ce9d8964b
md"""
- **Comprobar que la velocidad vsol(t) para t=50 is muy próxima a $-v_T$.** (El signo negativo de la velocidad terminal se refiere al sentido descendente del movimiento de la pelota).
"""

# ╔═╡ 4015e410-4df3-46a4-acff-274aea3f4203
vsol(50.,vT)+vT

# ╔═╡ b5668ad2-7cd0-43ce-8b94-1f27f57e609d
md"""
>**Comprobación**
>
...
# ╔═╡ 7d9208e0-e996-4533-83e9-52bb1951153a
md"""
...
# ╔═╡ 207090be-e5f0-4d2d-a100-9e5db69eb2bc
md"""
### 1.4- Ejercicio
- **Comprobar que efectivamente $vsol(0)=0$.**
"""

# ╔═╡ 9eb3e171-3e32-428c-a3f7-cc199f976d30
vsol(0,vT)

# ╔═╡ 711b525d-1a4e-4843-992f-53f7ccf5e4cb
md"""
### 1.5- Ejercicio
...
- **Para empezar, una vez fijados los valores de los parámetros constantes de problema dados en [(4)](#Par-Tenis), definir una función (que llamaremos $f$) que toma como argumentos de entrada la velocidad $v$ y devuelve como valor de salida el lado derecho de la ecuación diferencial [(1)](#Edo-Simp). Probar la función f implementada evaluando f(-vT).**
"""

# ╔═╡ 8ca4b514-bf67-456b-95c5-28d318ae3fe1
begin
 f(v) = -g - c/M*abs(v)*v # Las funciones sencillas se pueden definir en una línea
 f(-vT)
end

# ╔═╡ 1bf40188-ab46-42d8-8f93-fb096d0860ba
md"""
>**Comprobación**
>
...
# ╔═╡ 3bfbf0e2-edd7-466c-9b8c-424780b9b0be
md"""
...
# ╔═╡ 8424bb72-b8e0-4d2d-88d0-5cd6d1b6fd24
G(t,epsilon,vT) = (vsol(t+epsilon,vT) -vsol(t,vT))/epsilon - f(vsol(t,vT))  

# ╔═╡ 2695f11e-53e1-4771-9f20-478f1c812a1d
md"""
...
# ╔═╡ 23867842-dcce-4d27-8736-5a78204c021c
md"""
ϵ = $(ϵ_slider = @bind ϵ Slider(0.0001:0.001:0.01, show_value=true))
"""

# ╔═╡ cad1d865-dcfb-4fdc-be21-cd40397d5a80
let
 tF = 30
 tt = range(0,tF,length=1000)
 #ϵ = 0.01
 gg = G.(tt,ϵ,vT)
 plot(tt,gg, xlabel="t", label="ϵ=$ϵ", ylims=(0,0.01))
...
# ╔═╡ f4e6e4e0-c49c-4bc6-91fb-3480a032bb6a
md"""
...
# ╔═╡ ea03cf8d-2dab-4f33-830e-0bf3e64a6acb
md"""
>**Respuesta**
> Los resultados son coherentes con que el límite de G(t, ε) cuando ε → 0 sea 0 para cada t ∈ [0, 30] , y que por tanto v = vsol(t) satisface la ecuación diferencial (1).
"""

# ╔═╡ 5d8759f3-c176-4445-9fe2-36f35cc43e5f
md"""
## 2. Modelo más realista
"""

# ╔═╡ 02f27fd7-8f9d-4825-8fbc-ca9d67c5d16e
md"""
...
# ╔═╡ 8e3d51ce-a670-4b28-9244-2d2c055f5d4b
md"""
...
# ╔═╡ 7e9e13b1-bfbe-402c-8c32-43b49bf9ac62
function ac(v,p)
  M=p[1]
  g=p[2]
  R=p[3]
...
# ╔═╡ 45dae5fe-1977-49fd-8166-69b691833dab
md"""
...
# ╔═╡ 894439e2-6fb3-4c1d-b5d4-c7dfa4a5e6fb
begin

	
#    Multiple definitions for M and g !!!!
...
# ╔═╡ 7b6444ee-32cf-4e78-9681-942c89d6ef05
begin
	tF2 = 20
	n = 4000
	h = tF2/n
	...
	plot(tt2,vv, title = "Evolución de la velocidad de una pelota de ping-pong", 
            xlabel="t", ylabel="v", label="")

end

# ╔═╡ 3795aea0-bf05-4e97-bbd7-51e7ec163264
md"""
### 2.3- Ejercicio
...
# ╔═╡ ae36a027-4880-46c4-a507-65466d695904
vv[end-6:end]

# ╔═╡ e1486b47-7568-4fcc-bcf9-8192dde9754a
md"""
>**Respuesta**
> Efectivamente, la velocidad en los últimos seis tiempos de la discretización temporal coincide. 
"""

# ╔═╡ e0e0a6c5-a3e9-4f96-9a61-0e598a24c93d
md"""
...
# ╔═╡ a723a328-f6ff-488e-97a5-ff27d2e4623d
ac(vv[end],p)

# ╔═╡ 7db421bf-b547-4720-b5b8-b0a7e8fadef7
md"""
- **Es dicha acceleración total efectivamente nula? Comentar el resultado obtenido.**
"""

# ╔═╡ ef41e1bb-4cdc-4a5c-8267-b8e40536ad2d
md"""
>**Respuesta**
>La acceleración de la pelota para dicha velocidad es prácticamente nula (no es exactamente 0, pero parece que es debido a los errores de redondeo)
"""

# ╔═╡ 59981532-0661-4a83-96f5-4fb346748847
md"""
...
>**Respuesta**
> Tomaremos como velocidad terminal $v_T$ de la pelota de ping-pong la velocidad $v(t)$ en $t=20$ de la solución numérica (de [(5)](#Edo-Comp) con la condición inicial [(2)](#Cond-Ini) calculada previamente y guardada en vv[end]. Determinaremos el valor de $c$ imponiendole que $v_t = \sqrt{\frac{g*M}{c}}$, es decir $c = g*M/v_T^2$.

"""

# ╔═╡ 4d26f76a-e7e7-4f76-85d4-373cb7c22bb9
md"""
- **Calcular dicho valor de c**
"""

# ╔═╡ a8c859f5-8321-458b-ba7a-4568d118205e
begin
 vT2 = vv[end]
 c2 = g*M2/abs(vT2)^2
end

# ╔═╡ 8e6a6bb5-0286-44d1-bb77-6925dda7af3e
md"""
>**Comprobación**
>
...
# ╔═╡ 8987e5b1-9c56-4a6c-8d75-60a647dc8940
md"""
### 2.5- Ejercicio
...
# ╔═╡ 10167230-d505-4178-887e-d9cb8bdd4809
begin
 vs = vsol.(tt2,vT2);  # Velocidad de la pelota, según la solución del modelo simplificado
         # en los tiempos guardados en tt
 plot(tt2,vv, title = "Evolución de la velocidad de una pelota", 
            xlabel="t", ylabel="v", label="Modelo realista (Euler)")
 plot!(tt2,vs, label="Modelo simplificado")
end

# ╔═╡ 5e72cb58-fa7b-4d75-bbab-d208ebfa6fa5
md"""
...
# ╔═╡ 9e9cc90c-c7a7-492c-a3ad-f95b8d8612cc
plot(tt2,vv-vs, title="Diferencia entre velocidades en ambos modelos", xlabel="t", label="")

# ╔═╡ c4a4d150-574d-4762-88a0-3086684636ae
md"""
### 2.6- Ejercicio
...
# ╔═╡ 92c85dbf-48e0-4170-9fda-5eee2c76d409
begin
mu2=1.69e-5
pe2= 0.9
p2=[M2,g,R,mu2,pe2]
...
vv3000[1] = 0.
for j in 1:n
    vv3000[j+1]=vv3000[j] + h * ac(vv3000[j],p2)
end
plot(tt2,vv, title = "Velocidad de una pelota de ping-pong", 
            xlabel="t", ylabel="v", label="0m de altitud")
plot!(tt2,vv3000, label="3000m de altitud")
end

# ╔═╡ fbaf28ea-725c-4567-be24-09fe01fbe703
md"""
- **¿Son las diferencias apreciables? Es la velocidad terminal distinta en un caso y en otro?**
"""

# ╔═╡ 48e6df6b-cb6b-408a-b43b-260f76d8b167
vv3000[end]

# ╔═╡ 7ea90d65-b6c8-4f8f-a346-c75b2323acaa
md"""
>**Respuesta**
>
>_La diferencia de la evolución de la pelota de ping-pong al nivel del mar y a 3000 metros de altura es patente. La velocidad terminal de la pelota de ping-pong a 3000m ($v_T \approx 10.49m/s$) es apreciablemente mayor  que al nivel del mar ($v_T \approx 9.08m/s$)._
"""
